...
class Net(nn.Module):
    ...
    def __init__(self, cfg: Any):
        ...
        self.preprocessing = MelSpectrogram(**cfg.spec_args)
        self.preprocessing = nn.Sequential(MelSpectrogram(**cfg.spec_args),AmplitudeToDB(top_db=100.))  
        self.preprocessing2 = nn.Sequential(MelSpectrogram(**cfg.spec_args_center),AmplitudeToDB(top_db=100.)) 
        ...
        self.backbone.conv_stem = torch.nn.Conv2d(self.cfg.in_channels, out_ch, 
    ...
    def forward(self, batch):
        ...
        center_start = (batch['center'] - self.cfg.enlarge_len//2).clamp(0, l -  self.cfg.enlarge_len)
        center_end = center_start + self.cfg.enlarge_len
        x_center = torch.stack([x[t,:,cs:ce] for t,(cs,ce) in enumerate(zip(center_start, center_end ))])
        bs, c, l2 = x_center.shape
        x = x.reshape(bs*c,l)
        x_center  = x_center.reshape(bs*c,l2)
        
        with torch.cuda.amp.autocast(enabled=False): 
            x = self.preprocessing(x)
            x_center = self.preprocessing2(x_center)
            
        bsc, h, w = x.shape
        
        centers2 = (w * batch['center'] / l).long()
        buff = int((w * self.cfg.cut_from_large_mel) // 2)
        bsc, h, w = x.shape
        bsc, h, w2 = x_center.shape
        x = x.reshape(bs,c,h,w)
        x_center = x_center.view(bs,c,h,w2)
        centers_start2 = (centers2 - buff).clamp(0, w -  buff*2 - 1)
        centers_end2 = centers_start2 + buff*2 
        x = torch.stack([torch.cat((m[:,:,:cs], m_c,m[:,:,ce:]),2) \
                         for m,m_c,cs,ce in zip(x, x_center, centers_start2, centers_end2)])
